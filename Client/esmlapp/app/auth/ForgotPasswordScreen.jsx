  ...
  const handleNextStep = async () => {
    if (step === 1) {
      try {
        ...
        showCustomAlert('Success', 'Password reset request sent. Check your email for the code.');
        setStep(2);
      } catch (error) {
        showCustomAlert('Error', 'Error sending password reset request. Please try again.');
      }
    } else if (step === 2) {
      try {
        await axios.post(`${BASE_URL}/password/verify-reset-code`, { email, code });
        showCustomAlert('Success', 'Code verified. You can now reset your password.');
        setStep(3);
      } catch (error) {
      ...
      if (newPassword === repeatPassword) {
        try {
          ...
          showCustomAlert('Success', 'Password reset successfully. You can now log in.');
          navigation.navigate('Login');
        } catch (error) {
          showCustomAlert('Error', 'Error resetting password. Please try again.');
        }
      } else {
    ...
    <LinearGradient colors={['#fff', '#fff']} style={styles.gradient}>
      ...
      <SafeAreaView style={styles.container}>
        ...
        <Text style={styles.title}>
          {step === 1 ? 'Forgot Password?' : step === 2 ? 'Verify' : 'Setup New Password'}
        </Text>

        {step === 1 && (
          <View style={styles.inputContainer}>
            <Text style={styles.subtitle}>Don't worry! Please enter the email address linked with your account</Text>
            <TextInput
              style={styles.input}
