...
const EventDetails = () => {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        const response = await axios.get(`http://192.168.163.101:3000/events/getById/${eventId}`);
  ...
  const handleAddParticipant = async () => {
    try {
      ...
      await axios.post('http://192.168.163.101:3000/events/addParticipant', {
        eventId,
        userId,
      });

      Alert.alert('Success', 'You have been added to the event!');
      const updatedEvent = await axios.get(`http://192.168.163.101:3000/events/getById/${eventId}`);
